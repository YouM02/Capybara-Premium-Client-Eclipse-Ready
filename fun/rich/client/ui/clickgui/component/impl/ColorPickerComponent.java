...
public class ColorPickerComponent extends ExpandableComponent implements PropertyComponent {
    ...
    public void drawComponent(ScaledResolution scaledResolution, int mouseX, int mouseY) {
        ...
        mc.rubik_16.drawStringWithShadow(getName(), x + 2, y + height / 2F - 3, textColor);
        RenderUtils.drawSmoothRect(left, top, right, bottom, colorSetting.getColorValue());
        if(!ClickGUI.potato_mode.getBoolValue()) {
            RenderUtils.drawBlurredShadow(left - 2, top - 4, width - 89, height / 2.0F + 2, 5, RenderUtils.injectAlpha(new Color(colorSetting.getColorValue()), 50));
        }
        if (isExpanded()) {
            // Draw Background
            Gui.drawRect(x + fun.rich.client.ui.clickgui.Panel.X_ITEM_OFFSET, y + height, x + width - Panel.X_ITEM_OFFSET, y + getHeightWithExpand(), new Color(20, 20, 20, 160).getRGB());
